...
function ns.CreateAbout(info)
    ...
    -- Reglas del juego: el panel es también la ayuda
    local y = SectionHeader(panel, -160 - desc:GetStringHeight() - 16, L.HOW_TO_PLAY)
    local rules = panel:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
    rules:SetPoint("TOPLEFT", MARGIN_X + 8, y)
    rules:SetWidth(552)
    rules:SetJustifyH("LEFT")
    rules:SetSpacing(3)
    rules:SetText(L.RULES)
    ...
    for _, entry in ipairs(info.commands) do
        ...
        text:SetPoint("TOPLEFT", MARGIN_X + 270, y)
        text:SetWidth(290)
